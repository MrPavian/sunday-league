  ...
  render() {
    ...
    this.root.innerHTML = `
      <div class="settings-panel">
        ...
        <div class="choice">${[['kurz', tr('2 × 2 Minuten', '2 × 2 minutes')], ['mittel', tr('2 × 3 Minuten', '2 × 3 minutes')], ['lang', tr('2 × 5 Minuten', '2 × 5 minutes')]].map(([id, label]) => `<button class="${s.length === id ? 'active' : ''}" data-action="length" data-value="${id}">${label}</button>`).join('')}</div>
        <p class="hint">${tr('Gilt für alle Spiele, auch die simulierten der Liga. Turnierspiele sind etwas kürzer. Die Kraft ist auf die Spieldauer abgestimmt – müde werden die Jungs so oder so.', 'Applies to every match, including the simulated league games. Cup matches are a little shorter. Stamina is scaled to the length – the lads get tired either way.')}</p>
